...
{ 
    ...
    unsigned char signature[512] = "person-sign";
    unsigned char hash[32];
    mbedtls_md( mbedtls_md_info_from_type( MBEDTLS_MD_SHA256 ), signature, 16, hash);
    unsigned char buf[MBEDTLS_MPI_MAX_SIZE];

    unsigned char buff[10];
    unsigned char hash_receive[1024];

    for (int i=0;i<256;i++) {
        read(connfd, buff, 1);
        hash_receive[i] = buff[0];
    }
    ...
    for(int i = 0; i < 1024; i++ )
        printf("%02X%s", hash_receive[i],
                       ( i + 1 ) % 64 == 0 ? "\r\n" : " " );

    printf("\n\nCiphered data received :\n");
    for(int i = 0; i < 1024; i++ )
        printf("%02X%s", cipheredText[i],
                       ( i + 1 ) % 64 == 0 ? "\r\n" : " " );                   

    ret = mbedtls_rsa_pkcs1_verify( &rsa, NULL, NULL, MBEDTLS_RSA_PUBLIC,
                                  MBEDTLS_MD_SHA256, 20, hash, hash_receive );
  
    mbedtls_rsa_free( &rsa ); mbedtls_mpi_free( &N ); mbedtls_mpi_free( &E );
